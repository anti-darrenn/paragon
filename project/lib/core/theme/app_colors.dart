...
class AppColors {
  ...
  static const primaryDark = Color(
    0xFFCC5F00,
  ); // Pressed shade — primary at 80% (~20% darker)
  ...
  static const correct = Color(
    0xFF3FB950,
  ); // green — correct answer · Figma: success
  static const wrong = Color(
    0xFFF85149,
  ); // red   — wrong answer   · Figma: error
  static const warning = Color(0xFFD29922);
  static const submitGreen = Color(
    0xFF238636,
  ); // exam submit CTA — distinct from `correct` · Figma: success-overlay
  ...
  static const overlay = Color(
    0xFF000000,
  ); // Figma: overlay — apply with reduced opacity at call site (e.g. overlay.withAlpha(...))

  // ── Button fills ───────────────────────────────────────────────────
  static const buttonLight = Color(0xFFD9D9D9); // Figma: btn/google fill

  // ── Third-party brand colors (fixed — never theme these) ─────────
  static const googleBlue = Color(
    0xFF4285F4,
  ); // Google "G" mark, per Google brand guidelines
  ...
  static Color forSubject(String subject) {
    switch (subject.toLowerCase()) {
      ...
      case 'further mathematics':
        return subjectFurtherMathematics;
      case 'biology':
      ...
      case 'government':
        return subjectGovernment;
      case 'english language':
        return subjectEnglishLanguage;
      case 'music':
        return subjectMusic;
      case 'commerce':
        return subjectCommerce;
      default:
        return primary;
    }
  }
